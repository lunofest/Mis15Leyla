...
volumeBar.addEventListener('input', () => {
    ...
    if (volumeBar.value === '0') {
        volumeIcon.src = 'assets/vectores/mute.svg'; // Cambia a icono de mute
    } else {
        volumeIcon.src = 'assets/vectores/volume.svg'; // Icono normal
    }
});
...
// ---------------asistencia
    document.getElementById('confirmacion').addEventListener('click', function() {
        ...
        const phoneNumber = "541158672239";
        // Mensaje predefinido
        const message = "Holaaa, gracias por tu invitación, quiero confirmar mi asistencia\n*MI NOMBRE ES*: ";
    ...
    // ------------menu
        ...
        // Configuración para el botón "menu"
        document.getElementById('menu').addEventListener('click', function() {
          const phoneNumber = "541158672239"; // Número de WhatsApp
          const message = "Holaaa, gracias por pensar en mí, mi nombre es... y mi tipo de alimentacion es..."; // Mensaje predefinido
          const encodedMessage = encodeURIComponent(message); // Codifica el mensaje
          const whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`; // URL de WhatsApp
      ...
      // ---------------- paylist
         ...
         // Configuración para el botón "canciones"
    document.getElementById('canciones').addEventListener('click', function() {
      const phoneNumber = "541140631424"; // Número de WhatsApp
      const message = "Hola, la canción que no puede faltar es:"; // Mensaje predefinido
      const encodedMessage = encodeURIComponent(message); // Codifica el mensaje
      const whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`; // URL de WhatsApp
      window.open(whatsappURL, '_blank'); // Abre WhatsApp en una nueva pestaña
  });
...
function updateTimer() {
  const targetDate = new Date("january 25, 2025 20:00:00").getTime();
